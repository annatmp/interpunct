    ...
    def roulette_wheel_selection(self):
        ...
        if len(possible_sentences) == 0:
            ...
            if codeOfnewRule == self.dynamicNet.current.ruleCode:
                ...
                possible_sentences = SentenceRule.objects.filter(rule=rule_obj[0], sentence_active =True).all()
                containingSentences = dict()
                #and check how many other rules are included in this
                for sentence in possible_sentences:
                    contained_rules = list()
                    #store it in a dict
                    for i in sr.sentence.rules.all():
                        contained_rules.append(i.code)
                    containingSentences.update({sentence,contained_rules})

                #and find the one with the smallest set of other rules
                next_sentence = possible_sentences[0]
                for sentence in containingSentences:
                    if len(containingSentences[sentence]) < containingSentences[next_sentence]:
                        next_sentence = sentence
                return next_sentence

            # HACK: No sentence? Try again # TODO: find a real solution
